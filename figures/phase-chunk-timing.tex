\begin{tikzpicture}[
  ...
  \node[note,anchor=south west,text=red!60!black] at (5.45,3.65)
    {精细操作阶段\(\phi_t=\mathrm{fine}\)：每隔\(s_{\mathrm{fine}}\)步启动推理，\\旧块尾部（浅色）引导RTC生成新块};
  % 时间轴
  ...
  \node[note,anchor=north,text width=38mm] at (10.7,-0.55) {RTC在推理期间继续执行旧块，返回后切换到连续的新动作块};
\end{tikzpicture}
